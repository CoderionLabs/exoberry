...
//TODO: page_init and page_lookup
#include <inc/x86.h>
#include <inc/mmu.h>
#include <inc/error.h>
...
// This simple physical memory allocator is used only while DEOS is setting
...
boot_alloc(uint32_t n)
{
	...
	struct PageInfo *pp;	// Just used to get number of free pages
	char *result;
	int nfree; 				// This is the number of free pages
...
mem_init(void)
{
	...
	// create initial environment directory
	...
	// The kernel uses this earray to keep track of physical pages: for
...
page_init(void)
{
	...
	int num_alloc = ((int) boot_alloc(0) - KERNBASE) / PGSIZE;
	int iohole = 96;
	...
	for(i = npages_basemem; i < num_alloc + iohole + npages_basemem; i++){
		pages[i].pp_ref = 1;
	}
	for(; i < npages; i++){
		pages[i].pp_ref = 0;
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref != 0){panic("Page failed");};
